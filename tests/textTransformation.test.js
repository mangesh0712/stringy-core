...
describe('truncateWords', () => {
	test('truncates a string longer than wordCount and appends the ending', () => {
		const text = 'This is a long string that needs to be truncated';
		expect(truncateWords(text, 4, '...')).toEqual('This is a long...');
	});

	test('returns the string unchanged when it has wordCount or fewer words', () => {
		const text = 'Short string';
		expect(truncateWords(text, 4, '...')).toEqual('Short string');
	});

	test('uses a default ending when none is provided', () => {
		const text = 'This is a long string that needs to be truncated';
		expect(truncateWords(text, 4)).toEqual('This is a long...');
	});

	test('returns an empty string for non-string input', () => {
		expect(truncateWords(1234, 4)).toEqual('');
	});
});
